...
proctype server(byte serverId) {
	...
	byte votedFor = NIL;
	leader[serverId] = 0;// Initialize leader flag
	connect[serverId] = NUM_SERVERS - 1;// Initialize connection count
...
atomic {
	...
	leader[serverId] = 1;// Set leader flag
	leaders++;// Increment leader count
	isLeader = 1;// Set global leader flag
...
// update terms
	...
	:: (requestVote.term > currentTerm[serverId]) -> 
		...
		if (state[serverId] == leader) {
				leaders--;// Decrement leader count
				isLeader = (leaders > 0);// Update global leader flag
				leader[serverId] = 0;// Clear leader flag
			}
			state[serverId] = follower;
			votedFor = NIL
		:: (requestVote.term <= currentTerm[serverId]) -> 
