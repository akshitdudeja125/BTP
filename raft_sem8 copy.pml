...
#define MAX_STEPS 50// Maximum steps per server
#define STABLE_TIME 3// Consider system stable after this many consecutive stable rounds
...
byte steps[NUM_SERVERS];// Count of steps taken by each server
byte stable_rounds = 0;// Count of rounds where system has been stable
bool terminating = false;// Flag to coordinate termination
...
proctype server(byte serverId) {
	...
	steps[serverId] = 0;// Initialize step counter
...
// helpers
	...
	do// main loop
// Break loop if we've hit max steps or model is terminating
:: (steps[serverId] >= MAX_STEPS || terminating) -> 
	printf("SERVER %d: Terminating after %d steps\n",serverId,steps[serverId]);
	break;
// ... existing code for crash ...
:: // crash
	(state[serverId] != CRASHED && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // timer ticks down (timeout count down)
	(state[serverId] != CRASHED && state[serverId] != LEADER && time_out[serverId] > 0 && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // timeout occurs (time_out reaches 0)
	(state[serverId] == FOLLOWER && time_out[serverId] == 0 && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // candidate election completion check
	(state[serverId] == CANDIDATE && time_out[serverId] == 0 && !terminating) -> 
	atomic {
		steps[serverId]++;
// Count votes
		voteCount = 0;
...
:: // restart
	(state[serverId] == FOLLOWER && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // handle incoming messages
	(toNodes[serverId]?[msg] && state[serverId] != CRASHED && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // heartbeat (leader sends to followers)
	(state[serverId] == LEADER && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // client request
	(state[serverId] == LEADER && logs[serverId].logs[1] == 0 && !terminating) -> 
	atomic {
		steps[serverId]++;
		if
		:: logs[serverId].logs[0] == 0 -> 
...
:: // stable leader with committed entries
	(state[serverId] == LEADER && commitIndex[serverId] > 0 && !terminating) -> 
	atomic {
		steps[serverId]++;
		printf("SERVER %d: Stable leader with committed entries\n",serverId);
// Check if all followers have the commit
		byte committedFollowers = 0;
		for (i : 0 .. NUM_SERVERS - 1) {
			if
			:: (i != serverId && state[i] == FOLLOWER && commitIndex[i] > 0) -> 
				committedFollowers++;
			:: else -> skip;
			fi
		}
		
// If we have a majority committed, count as stable
		if
		:: (committedFollowers > (NUM_SERVERS / 2)) -> 
			if
			:: (serverId == 0) -> // Only increment once per round
				stable_rounds++;
				printf("SYSTEM: Stable round %d / %d detected\n",stable_rounds,STABLE_TIME);
				if
				:: (stable_rounds >= STABLE_TIME) -> 
					printf("SYSTEM: Stability goal reached,signaling termination\n");
					terminating = true;
				:: else -> skip;
				fi
			:: else -> skip;
		...
		:: else -> 
			if
			:: (serverId == 0) -> // Only reset once per round
				stable_rounds = 0;
			:: else -> skip;
			fi
		fi
...
:: // stable follower with updated logs
	(state[serverId] == FOLLOWER && commitIndex[serverId] > 0 && !terminating) -> 
	atomic {
		steps[serverId]++;
...
:: // candidate with majority votes
	(state[serverId] == CANDIDATE && !terminating) -> 
	atomic {
		steps[serverId]++;
// Count votes
		voteCount = 0;
...
atomic {
	for (i : 0 .. NUM_SERVERS - 1) {
		...
		steps[i] = 0;// Initialize step counters
		run server(i);
	}
}
...
// Existing LTL properties
ltl liveness { <> (isLeader == 1) }

// New property for termination
ltl termination { <> terminating }

// New property to verify stable system
ltl stability { <> (stable_rounds >= STABLE_TIME) }
