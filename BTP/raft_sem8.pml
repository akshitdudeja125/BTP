#define MAX_TERM 2// Reduced from 3
#define MAX_LOG 1// Reduced from 2
#define NUM_SERVERS 2// Reduced from 3
#define MSG_CAPACITY 3// Reduced from 5
...
proctype server(byte serverId) {
	...
	byte votedFor = NIL;
	time_out[serverId] = serverId + 1;// Different timeouts to avoid conflicts
...
// Check votes immediately - artificial but helps state exploration
		voteCount = 0;
		for (i : 0 .. NUM_SERVERS - 1) {
			if
...
// If we have enough votes already (our own), become leader
		if
...
:: // stable leader with committed entries
	(state[serverId] == LEADER && commitIndex[serverId] > 0) -> 
	atomic {
...
:: // stable follower with updated logs
	(state[serverId] == FOLLOWER && commitIndex[serverId] > 0) -> 
	atomic {
...
#define ELECTION_SAFETY(id1,id2) [](!((state[id1] == LEADER && state[id2] == LEADER && currentTerm[id1] == currentTerm[id2])))


ltl electionSafety {
ELECTION_SAFETY(0,1) && ELECTION_SAFETY(0,2) 
&& ELECTION_SAFETY(1,2) 
...
// Simplified approach: Check that if a node is crashed, it's possible for it to become a leader in the future
// This avoids nested eventuality operators that can cause state explosion
ltl eventualLeadership {
<> (state[0] == CRASHED && <> (state[0] == LEADER)) || 
<> (state[1] == CRASHED && <> (state[1] == LEADER)) || 
<> (state[2] == CRASHED && <> (state[2] == LEADER))
}
...
ltl liveness { <> (leaders > 0) }
// Property 3: Uniqueness - At most one leader per term
ltl uniqueness { [] (leaders <= 1) }

// Heartbeat Effectiveness: If a follower has non-zero timeout (meaning it recently received a heartbeat)
// and there's a leader in the same term, it should not become a candidate
#define HEARTBEAT_EFFECTIVENESS(id,leader_id) [] \
( \
(state[id] == FOLLOWER && time_out[id] > 0 && \
state[leader_id] == LEADER && currentTerm[id] == currentTerm[leader_id]) -> \
X(state[id] != CANDIDATE) \
)

ltl heartbeatEffectiveness {
HEARTBEAT_EFFECTIVENESS(0,1) && HEARTBEAT_EFFECTIVENESS(0,2) && 
HEARTBEAT_EFFECTIVENESS(1,0) && HEARTBEAT_EFFECTIVENESS(1,2) && 
HEARTBEAT_EFFECTIVENESS(2,0) && HEARTBEAT_EFFECTIVENESS(2,1)
}

// Heartbeat Leadership Stability: A leader that successfully sends heartbeats to the majority
// of servers should remain the leader unless it crashes
#define HEARTBEAT_STABILITY(id) [] \
( \
(state[id] == LEADER && connect[id] >= (NUM_SERVERS / 2 + 1)) -> \
(state[id] == LEADER W state[id] == CRASHED) \
)

ltl heartbeatStability {
HEARTBEAT_STABILITY(0) && HEARTBEAT_STABILITY(1) && HEARTBEAT_STABILITY(2)
}
